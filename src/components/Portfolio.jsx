                ...
                {portfolios.map((portfolio)=>(
                    <div className="shadow-md shadow-gray-600 rounded-lg" key={portfolio.id}>
                        <img src={portfolio.src} alt="" 
                            className="rounded-md duration-500 w-full h-3/4 hover:scale-105"/>
                        <div className="flex items-center justify-between h-1/4">
                        { portfolio.demoLink && (
                            <button onClick= {()=>onButtonClick(portfolio.demoLink)} className="w-1/2  duration-300 hover:bg-slate-800 hover:opacity-80 p-4 rounded-md">Demo Link</button>
                        )}
                            <button onClick= {()=>onButtonClick(portfolio.codeLink)} className={`${portfolio.demoLink ? "w-1/2" : "w-full text-lg"}  hover:bg-slate-800 hover:opacity-80 p-4 rounded-md`}>Source Code</button>
                        </div>
                    </div>
                ))}
            </div>
